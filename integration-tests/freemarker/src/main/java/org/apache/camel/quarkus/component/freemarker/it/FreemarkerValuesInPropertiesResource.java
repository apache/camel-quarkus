...
public class FreemarkerValuesInPropertiesResource {
    ...
    @Path("/testVelocityLetter")
    @POST
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.TEXT_PLAIN)
    public String testVelocityLetter() throws Exception {
        Exchange exchange = producerTemplate.request("direct:freemarkerValuesInProperties", new Processor() {
            @Override
    ...
    public static class FreemarkerRouteBuilder extends RouteBuilder {
        ...
        public void configure() {
            from("direct:freemarkerValuesInProperties")
                    .to("freemarker:dummy");
        }
    }
}
