...
public class BoxRouteBuilder extends RouteBuilder {
    ...
    public void configure() {
        ...
        from("direct:upload-file")
                .to("box://" + PATH_PREFIX + "/uploadFile?check=true");

        from("direct:download-file")
                .to("box://" + PATH_PREFIX + "/downloadFile?inBody=fileId");

        from("direct:delete-file")
                .to("box://" + PATH_PREFIX + "/deleteFile?inBody=fileId");
