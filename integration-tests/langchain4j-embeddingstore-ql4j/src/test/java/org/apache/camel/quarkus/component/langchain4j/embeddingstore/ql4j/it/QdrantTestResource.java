...
public class QdrantTestResource implements QuarkusTestResourceLifecycleManager {
    ...
    private static final String COLLECTION_NAME = "camel-quarkus-ql4j-collection";
    ...
    private void createCollection(String host, int port) {
        try {
            ...
            String url = String.format("http://%s:%d/collections/%s", host, port, COLLECTION_NAME);
            // Vector size must match the embedding dimension in the WireMock stubs under mappings/embeddings/
            String body = """
                    {
                      "vectors": {
                        "size": 5,
            ...
            LOG.info("Created Qdrant collection '{}': {}", COLLECTION_NAME, response.body());
