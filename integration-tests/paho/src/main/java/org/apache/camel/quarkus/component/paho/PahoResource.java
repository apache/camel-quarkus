...
public class PahoResource {
    ...
    public String consumePahoMessage(@PathParam("protocol") String protocol, @PathParam("queueName") String queueName) {
        if ("ssl".equals(protocol)) {
            setKeyStore(KEYSTORE_FILE, KEYSTORE_PASSWORD);
        }
        try {
            return consumerTemplate.receiveBody("paho:" + queueName + "?brokerUrl=" + brokerUrl(protocol), 5000, String.class);
        } finally {
            if ("ssl".equals(protocol)) {
                removeKeyStore(KEYSTORE_FILE);
            }
        }
    }
    ...
    public Response producePahoMessage(@PathParam("protocol") String protocol, @PathParam("queueName") String queueName,
        ...
        try {
            producerTemplate.sendBody("paho:" + queueName + "?retained=true&brokerUrl=" + brokerUrl(protocol), message);
        } finally {
            if ("ssl".equals(protocol)) {
                removeKeyStore(KEYSTORE_FILE);
            }
    ...
    private void setKeyStore(String keystore, String password) {
        ...
        try {
            ...
            LOG.error("An exception occurred while copying the keystore, can't test with SSL", e);
    ...
    private void removeKeyStore(String keystore) {
        try {
            ...
            LOG.warn("An exception occurred while deleting the keystore", e);
        } finally {
