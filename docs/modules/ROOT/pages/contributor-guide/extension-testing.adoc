...
* This rule also applies to `junit`, `testcontainers`, `rest-assured`, `assertj` and similar testing dependencies:
  Unless there is a good reason to do the opposite, they should be kept in Maven `test` scope.
...
`io.quarkus.test.QuarkusExtensionTest` may suit well for such situations.
