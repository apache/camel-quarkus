...
For instance, would an issue occurs while performing the release of `camel-quarkus-openapi-java`, first of all you need to identify which directori `release:perform` used for the release process, usually it is `<root_of_the_project>/target/checkout`:
