...
A store or model that exists only under a name, for example with
`quarkus.langchain4j.pgvector.default-store-enabled=false`, cannot serve the default augmentor.
Such an application fails at startup with a message pointing at
