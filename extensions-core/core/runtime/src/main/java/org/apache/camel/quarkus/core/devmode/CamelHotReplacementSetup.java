    ...
    public void setupHotDeployment(HotReplacementContext context) {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                // Skip if continuous testing is currently running
                // Triggering a restart during test execution can cause the DevUI continuous testing
                // infrastructure to break with NoSuchElementException when trying to re-initialize
                // See: https://github.com/apache/camel-quarkus/issues/8702
                try {
                    ContinuousTestingSharedStateManager.State testingState = ContinuousTestingSharedStateManager
                            .getLastState();
                    if (testingState != null && testingState.inProgress) {
                        LOG.trace("Skipping scan - continuous testing is currently running");
                        return;
                    }
                } catch (NoClassDefFoundError e) {
                    // ContinuousTestingSharedStateManager may not be available in all scenarios
                    // (e.g., older Quarkus versions or when continuous testing is not on classpath)
                    LOG.trace("ContinuousTestingSharedStateManager not available - skipping check");
                }
