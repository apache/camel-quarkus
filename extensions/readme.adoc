...
`platform-http:path` | 0.3 | HTTP service leveraging existing runtime platform HTTP server
...
`sjms2:destinationType:destinationName` | 0.5 | The sjms2 component (simple jms) allows messages to be sent to (or consumed from) a JMS Queue or Topic (uses JMS 2.x API).
...
| link:https://camel.apache.org/components/latest/tidyMarkup-dataformat.html[TidyMarkup] (camel-quarkus-tagsoup) | 0.5 | TidyMarkup data format is used for parsing HTML and return it as pretty well-formed HTML.
...
| (camel-quarkus-telegram) | 1.0 | Telegram Chat Support
|===
// others: END
