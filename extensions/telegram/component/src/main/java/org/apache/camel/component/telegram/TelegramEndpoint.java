...
public class TelegramEndpoint extends ScheduledPollEndpoint implements WebhookCapableEndpoint {
    ...
    protected void syncProxy() {
        if (telegramService != null && ObjectHelper.isNotEmpty(configuration.getProxyHost())
            ...
            telegramService.setHttpProxy(configuration.getProxyHost(), configuration.getProxyPort());
        }
    }
    ...
    public void registerWebhook() throws Exception {
        if (!telegramService.setWebhook(configuration.getAuthorizationToken(), webhookConfiguration.computeFullExternalUrl())) {
    ...
    public void unregisterWebhook() throws Exception {
        if (!telegramService.removeWebhook(configuration.getAuthorizationToken())) {
