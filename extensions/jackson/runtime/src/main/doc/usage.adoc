...
If you are using the JSON binding mode in the Camel REST DSL, the Quarkus Jackson `ObjectMapper` is automatically discovered by default (`autoDiscoverObjectMapper=true`). You can override this behavior if needed:
...
public class Routes extends RouteBuilder {
    public void configure() {
        restConfiguration().dataFormatProperty("autoDiscoverObjectMapper", "false");
