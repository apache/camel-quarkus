...
Note that this extension automatically enables native serialization support for you. Refer to the xref:user-guide/native-mode.adoc#serialization[native mode user guide] for more information.

=== Support for Connection pooling and X/Open XA distributed transactions

ifeval::[{doc-show-extra-content} == true]
include::camel-quarkus-extensions/jms-connection-pooling.adoc[]
endif::[]
...
ifeval::[{doc-show-extra-content} == true]
include::camel-quarkus-extensions/jms-connection-pooling-ibmmq.adoc[]
endif::[]
