...
NOTE: Amazon MSK is not supported by Floci, so the `Aws2TestEnvCustomizer` SPI (used by other AWS modules to start a Floci container) cannot be used here. WireMock is used instead to simulate the MSK API in the default (non-real AWS) test mode.
